    ...
    if name.startswith(events[0].name.split('_')[0]): # check if name of the folder is 'flegrei*'
        ...
        for ev in events:
            if ev.name==name:
                ...
                print('ERROR: ',ev.name,' not present in DATA')
        ...
        for trace in w:
            for s in st:
                if trace.stats.station==s.station:
                    st_coord.append( [trace.stats.station, trace.stats.channel , s.lat,s.lon ] ) #station, channel, lat, long
        ...
        for row in st_coord:
            ...
            dist_vs_amp.append( [ row[0], row[1],dist ] ) # station, channel, dist
