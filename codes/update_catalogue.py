...
stime=UTCDateTime('2024-11-01T00:00:00')        # CHANGE set start time
etime=UTCDateTime('2024-12-06T17:00:00')        # CHANGE set end time

print('downloading events from INGV database')
cat_INGV_new=client.get_events(starttime=stime,endtime=etime,
                        minlatitude=40.75,maxlatitude=40.90,minlongitude=14.00,
                        maxlongitude=14.20)

cat_mag_INGV_new = cat_INGV_new.filter("magnitude >= 2.5")
    
#cat_mag_INGV.plot(projection='local',resolution='i') #plot

print('Updating catalogue INGV in .xml')
print('number of new events selected:', len(cat_mag_INGV_new), 'out of:',len(cat_INGV_new))

def merge_catalogues_xml(cat1,cat2):
    for ev in cat2:
        cat1.append(ev)
    return cat1
...
cat_INGV=merge_catalogues_xml(cat_INGV,cat_INGV_new)
