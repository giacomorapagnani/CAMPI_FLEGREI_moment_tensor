...
### NOT WORKING: ONLY FEW STATIONS SHOWED, MAYBE ONLY FAR ONES
...
for file in os.listdir(datadir):
    ...
    if name.startswith(events[0].name.split('_')[0]): 
        ...
        for tr in w:
            for s in st:
                if tr.stats.station==s.station:
                    n_el_noise= int( round( len(tr.data)/140*40 ) )                     # take time interval [0,40]s of the entire leght (140s) to compute noise's rms
                    n_el_eq1= int( round( len(tr.data)/140*45 ) )                        # take time interval [45,105]s of the entire leght (140s) to compute eq's rms
                    n_el_eq2= int( round( len(tr.data)/140*105 ) )

                    rms_noise= num.sqrt( num.mean(tr.data[:n_el_noise]**2) )
                    rms_eq= num.sqrt( num.mean(tr.data[n_el_eq1:n_el_eq2]**2) )
                    st_coord.append( [tr.stats.station, tr.stats.channel , s.lat, s.lon , rms_eq , rms_noise ] ) #station, channel, lat, long, rms_eq, rms_noise
